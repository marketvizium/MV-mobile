    ...
    async cadastrarCotacao() {
      if (this.vendedoresSelecionados.length === 0) {
        this.$toast.add({
            ...
            detail: 'Selecione ao menos um vendedor para criar o pedido direto',
            life: 3000
            });
      ...
      try {
        ...
        await api.post(
          `/mvpu/cotacao/criarPedidoDireto/${this.auth.loja.id_loja}`,
          payload
        )
        ...
        if (this.$toast) {
          this.$toast.add({ 
            severity: 'success', 
            summary: 'Pedido direto criado', 
            detail: 'Novo pedido direto cadastrado com sucesso', 
            life: 3000 
            });
        }
